"""Point-in-time features, feature_version v1 (15-minute decision bars).
...
def rolling_percentile(values: list[float | None], window: int, min_obs: int | None = None) -> list[float | None]:
    """Percent of the last `window` bars' non-None values (current included) that are <= current."""
    ...
    for i, value in enumerate(values):
        ...
        if value is not None and len(ordered) >= min_obs:
            out[i] = 100.0 * bisect_right(ordered, value) / len(ordered)
    return out


def event_percentiles(events: list[tuple[int, float]], window_ms: int, min_obs: int) -> list[float | None]:
    ...
    for stamp, value in events:
        ...
        while queue and queue[0][0] <= stamp - window_ms:
            _, old = queue.popleft()
            del ordered[bisect_left(ordered, old)]
        out.append(100.0 * bisect_right(ordered, value) / len(ordered) if len(ordered) >= min_obs else None)
    return out
...
def align(pair: str, perp: list[RawRecord], spot: list[RawRecord], premium: list[RawRecord],
          funding: list[RawRecord], oi: list[RawRecord]) -> Frame:
    ...
    percentiles = event_percentiles(funding_events, WINDOW_MS, min_obs=30)
...
def compute(frame: Frame, lookbacks: tuple[int, ...] = (20, 55)) -> Frame:
    ...
    for name, bars in (("1h", 4), ("4h", 16)):
        ...
        f[f"oi_chg_{name}_pct"] = rolling_percentile(change, WINDOW_BARS)
    ...
    f["buy_ratio4_pct"] = rolling_percentile(buy_ratio, WINDOW_BARS)
    f["sell_ratio4_pct"] = rolling_percentile(sell_ratio, WINDOW_BARS)
    f["up_impact_pct"] = rolling_percentile(up_impact, WINDOW_BARS)
    f["down_impact_pct"] = rolling_percentile(down_impact, WINDOW_BARS)
    f["premium_pct"] = rolling_percentile(frame.premium, WINDOW_BARS)
