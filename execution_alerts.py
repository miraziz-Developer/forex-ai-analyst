...
def report(incident_key: str, message: str, *, severity: str = "WARNING", details: dict | None = None,
    ...
    try:
        ...
        rows = storage._rows_as_dicts(storage._execute(
            "SELECT notified_at FROM execution_incidents WHERE incident_key = ?", [incident_key]))
        notify = not rows
        if rows and rows[0].get("notified_at"):
            try:
        ...
        (incident_key, severity, message, details_json, first_seen_at, last_seen_at, occurrences, notified_at)
        VALUES (?, ?, ?, ?, ?, ?, 1, ?)
...
def status() -> dict:
    rows = storage._rows_as_dicts(storage._execute("""SELECT count(*) AS open_incidents,
        max(last_seen_at) AS last_incident_at FROM execution_incidents"""))
