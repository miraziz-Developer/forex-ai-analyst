...
- **Markets (20):** BTC, ETH, SOL, XRP, BNB (lab); DOGE, ADA, LINK, AVAX, LTC and DOT, TRX, BCH, UNI, NEAR, ATOM, ETC, FIL, AAVE, XLM, each group added only after passing a pre-registered out-of-sample test with unchanged parameters ([`docs/DONCHIAN_NEW_MARKETS.md`](docs/DONCHIAN_NEW_MARKETS.md), [`docs/DONCHIAN_TWENTY_MARKETS.md`](docs/DONCHIAN_TWENTY_MARKETS.md)). About 14 trades a month with the default cap.
- **Gates:** one position per pair, at most `MAX_CONCURRENT_POSITIONS` (12) open, `TRADE_COOLDOWN_MINUTES` (60) per pair after a close, each 4h breakout traded at most once, runtime kill switch and blocked pairs.
...
Evidence and its limits: [`docs/LAB_REPORT.md`](docs/LAB_REPORT.md) (walk-forward), the two out-of-sample market tests above, and three pre-registered studies of alternatives that all failed: partial take-profit, a swing-strategy combination (pullback, SMC/zigzag, ICT sweep, momentum) and BTC-regime filters/shorts ([`docs/DONCHIAN_PARTIAL_EXIT.md`](docs/DONCHIAN_PARTIAL_EXIT.md), [`docs/COMBINATION_STUDY.md`](docs/COMBINATION_STUDY.md), [`docs/REGIME_STUDY.md`](docs/REGIME_STUDY.md)). A one-account simulation of the twenty markets at 0.3% risk per trade and 12 open positions gave about 24% a year with a -15% maximum drawdown in 2021-2026, mostly from a few strong trend years; 0.5% risk gave about 38% with -24%. This is a backtest, not a promise: the strategy is a **VST forward test**, and after 179+ trials on this data no result clears a deflated-Sharpe test. Leverage does not change the risk per trade (the stop distance and quantity do); it only changes the margin used.
...
- **Degradation alarm:** every hour, closed Donchian trades are measured in R (P&L / risk taken) and compared with the backtest of the same configuration (1147 trades on the twenty markets: 31% wins, mean +0.51R, worst losing run 26, worst drawdown -64R). Half the historical worst (16 losses in a row or -32R) raises a WARNING; anything worse than the backtest's worst (27 in a row or -65R) raises a CRITICAL alert recommending `STOP`. It alerts only; the operator decides.
